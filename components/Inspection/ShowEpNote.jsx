...
export function ShowEpNote({ isOpen, onClose, InspectionID }) {
  ...
  const donwloadFiles = async (fileType) => {
    ...
    try {
      const response = await axiosInstance.get(`/epnote/download/${InspectionID}`, {
        responseType: 'blob',
      ...
      <div className="flex flex-col gap-4">
        <div className="flex gap-4 items-center">
          ...
          <div className="flex flex-col items-start gap-3">
            ...
            <h1 className="">{inspection.user.position} -- {inspection.user.departement}</h1>
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <h1 className="font-bold text-md">ouvrage</h1>
          <h1 className="text-md font-medium">
            <span className="">{epNote.name}</span> --- {inspection.ouvrage_type}
          </h1>
        </div>
        <div className="flex flex-col gap-2">
